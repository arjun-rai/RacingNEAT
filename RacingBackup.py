...
def main(genomes, config):
    ...
    running=True
    while running:
        if(len(pList)==0 or time.time()-t>=15):
            # for g in ge:
            #     g.fitness-=5
            running = False
            break
...
#             BxLeft, ByLeft = p.rect.bottomleft
            ...
            if (x<len(pList) and screen.get_at((int(xRight), int(yRight)))[:3]==(252,252,252) and p.check==1):
                p.check = 2
                #p.score+=1
                ge[x].fitness+=5
            if (x<len(pList) and screen.get_at((int(xRight), int(yRight)))[:3]==(255,255,255) and (p.check == 2 or p.check ==0)):
                p.check = 1
                #p.score+=1
                ge[x].fitness+=5
            ...
            if (x<len(pList) and math.degrees(p.heading)%360>-45 and math.degrees(p.heading)%360<45 and p.position.y>500):
                ge[x].fitness-=1
            elif(x<len(pList) and abs(math.degrees(p.heading))%360 > 135 and abs(math.degrees(p.heading))%360 < 225 and p.position.y<500):
                ge[x].fitness-=1


            if (x<len(pList) and screen.get_at((int(xRight), int(yRight)))[:3]==(255,0,0)  and p.point==True and math.degrees(p.heading)%360>-45 and math.degrees(p.heading)%360<45):
                players.remove(p)
                nets.pop(x)
                ge.pop(x)
                pList.pop(x)
            elif (x<len(pList) and screen.get_at((int(xLeft), int(yLeft)))[:3]==(255,0,0)  and p.point==True and math.degrees(p.heading)%360>-45 and math.degrees(p.heading)%360<45):
                players.remove(p)
                nets.pop(x)
                ge.pop(x)
                pList.pop(x)
    ...
    if (maxFit<max.fitness):
        draw_net(config, max, node_names={0:"Accelerate", 1:"Brake", 2:"Left", 3:"Right", -1: "dLeft", -2:"dForward", -3:"dRight", -4:"Speed", -5:"Heading"})
        maxFit = max.fitness
#main()
...
def run(config_path):
    ...
    winner = p.run(main,500) #50 = number of generations
